...
def Write_Hex_to_String_64(file_path,Device_Type_temp):
	...
	thefile.write(str(EEP_WR_CMD) + " "+ EEP_FDR_APP_VALID +" 240 "+str(DEF_EEP_DATA)+\n") # make application Invalid basically erases device type stored in FDR 
	thefile.write(str(EEP_WR_CMD) + " "+ EEP_FDR_APP_VERSN +" 240 "+str(DEF_EEP_DATA)+\n") # make application version 255 
	...
	thefile.write(str(EEP_WR_CMD) + " "+ EEP_FDR_APP_CHKSM + " 240 " + str(image_checksum) + "\n") # write image_checksum
	thefile.write(str(EEP_WR_CMD) + " "+ EEP_FDR_APP_VALID + " 240 " + str(Device_Type_temp) + "\n") # make application valid
	thefile.close()	
...
while (1) :
	...
	if ((option == 'K') or (option == 'k')):
        ...
        Fect_File_Path = raw_input("\nEnter File path to store fectory OTA file: ") #read hex file path	
        Device_Typ = raw_input("\nEnter Device  1:DEMO 2: Wall Switch ? : ") #read hex file path	
        Device_Typ = int(Device_Typ)
        if((Device_Typ > 0) and (Device_Typ < 3)):
           Write_Hex_to_String_64(Fect_File_Path,Device_Typ)
        else:
            print "Data Input Error"
        ...
        Device_Typ = int(Device_Typ)
        if((Device_Typ > 0) and (Device_Typ < 3)):
           Write_Hex_to_String_64(cred_File_Path)
        else:
            print "Data Input Error"		
